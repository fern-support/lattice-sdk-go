...
```go
response, err := client.OAuth2.GetToken(...)
if err != nil {
...
```go
response, err := client.OAuth2.WithRawResponse.GetToken(...)
if err != nil {
    return err
}
...
response, err := client.OAuth2.GetToken(ctx, ...)
```
...
response, err := client.OAuth2.GetToken(ctx, request, ...)
```
